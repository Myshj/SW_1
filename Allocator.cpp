...
/*Спроба видылення пам'яті заданого розміру.*/
...
void Allocator::MergeFreeWith(Block block){
    ...
    for(int i = 0; i < freeBlocks.size(); i++){
        if(freeBlocks[i].StartAddress() + freeBlocks[i].Size()  == temp.StartAddress()){
    ...
    for(int i = 0; i < freeBlocks.size(); i++){
        if(temp.StartAddress() + temp.Size() == freeBlocks[i].StartAddress()){
